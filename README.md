...
- **🔥 Peak / Off-Peak tariff tracking** — real-time monitoring of DeepSeek API rate tiers (Peak: 01:00–04:00 & 06:00–10:00 UTC at standard rates; Off-Peak: 50% discount on all tokens) with updated deterministic price thresholds for the latest `deepseek-flash` rates ($0.30 Peak / $0.15 Off-Peak per 1M tokens for input miss, $0.006 / $0.003 for cache hit, and $1.20 / $0.60 for output) alongside full backward compatibility for older V4 rates. Displayed via a badge dot on the tray icon (🔴 Red / 🟢 Green), tooltips, context menu, and a dedicated dashboard card with countdowns
...
- **⚡ DeepSeek-V4.1-Flash & Streamlined Dashboard** — native support for DeepSeek's new flagship `deepseek-flash` model (V4.1-Flash released Sept 2026) with lower pricing. The main dashboard features a sleek, compact single-row overview focused directly on Flash token usage and costs for both the selected month and today's spend, while multi-model historical breakdowns (Flash, Pro, Vision) remain available in the Detailed Statistics window.
...
- **🔥 Sledování tarifních špiček (Peak / Off-Peak)** — monitorování cenových pásem DeepSeek API v reálném čase (špička 01:00–04:00 a 06:00–10:00 UTC za plnou cenu, mimo špičku 50% sleva na všechny tokeny) s adaptivní detekcí upravenou pro nové cenové hladiny modelu `deepseek-flash` ($0.30 špička / $0.15 mimo špičku za 1M tokenů pro input miss, $0.006 / $0.003 pro cache hit a $1.20 / $0.60 pro output) s plným zachováním zpětné kompatibility pro starší data. Indikováno tečkou na tray ikoně (🔴 červená / 🟢 zelená), v tooltipu, v menu a na samostatném bloku dashboardu s odpočtem času
...
- **⚡ Podpora DeepSeek-V4.1-Flash a čistý Dashboard** — plná podpora nového vlajkového modelu `deepseek-flash` (V4.1-Flash vydaného v září 2026) s novými výhodnějšími cenami. Hlavní dashboard byl zjednodušen na kompaktní, čisté zobrazení modelu Flash v měsíčním i dnešním přehledu (historický rozpad starších modelů Pro a Vision zůstává plně k dispozici v Podrobných statistikách).
